...
import {
    ...
    startConnect,
} from './integrations.js?v=demo-integrations-2';
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function renderIntegrationsPanel() {
        ...
        integrationsStatusText.textContent = integrationState.connected
            ? `Connected${providerLabel ? ` via ${providerLabel}` : ''}${integrationState.email ? ` · ${integrationState.email}` : ''} (demo)`
            : 'Not connected — choose a provider to authorize Constellation (demo simulates OAuth).';
    ...
    async function initializePage() {
        ...
        if (globalState.currentUser) {
            ...
            await setupUserMenuAndAuth(supabase, globalState, { skipImpersonation: true });
