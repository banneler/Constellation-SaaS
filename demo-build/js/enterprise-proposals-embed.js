...
Proposal Studio and IRR workflows complete the motion by connecting internal strategy to customer-ready assets and investment review. The result is a cleaner enterprise selling system: fewer disconnected tools, faster follow-through, stronger executive alignment, and better visibility into where strategic attention is creating pipeline.` },
        ...
        function downloadSpecFile(projectData) {
            ...
            a.download = (projectData.globalBiz || 'Constellation_Proposal').replace(/\s+/g, '_') + '.spec';
            a.click();
        ...
        setTimeout(function loadDemoProposalOnDirectEntry() {
            var params = new URLSearchParams(window.location.search);
            if (params.get('load')) return;
            var demoRow = window.CONSTELLATION_DEMO_STATE?.tables?.proposal_specs?.[0];
            if (!demoRow || !demoRow.spec) return;
            _suppressDirty = true;
            loadedProposalId = demoRow.id;
            loadedProposalName = demoRow.name || null;
            loadedProposalAccountId = demoRow.account_id != null ? String(demoRow.account_id) : null;
            applySpecToForm(demoRow.spec);
            _suppressDirty = false;
            setDirty(false);
        }, 1300);
        ...
        function sendForProofing() {
            if (window.CONSTELLATION_DEMO_STATE) {
                showToast('Demo mode: proofing emails are disabled. Download the spec or PDF instead.', 'info');
                return;
            }
        ...
        function promptPdfFileName(defaultBase) {
            ...
            if (!suggestion) suggestion = 'Constellation_Proposal';
